...
export const syncServidoresToFirebase = async () => {
  ...
  try {
    ...
    const promises = servidores.map(async (servidor) => {
      const servidorRef = doc(db, SERVIDORES_COLLECTION, `${USER_ID}_${servidor.id}`)
...
export const syncClientesFromFirebase = async () => {
  ...
  try {
    isSyncing = true
    const q = query(collection(db, CLIENTES_COLLECTION), where('userId', '==', USER_ID))
    ...
    querySnapshot.forEach((docSnap) => {
      ...
      cliente.status = calcularStatus(cliente.dataVencimento)
...
export const syncServidoresFromFirebase = async () => {
  ...
  try {
    isSyncing = true
    const q = query(collection(db, SERVIDORES_COLLECTION), where('userId', '==', USER_ID))
    const querySnapshot = await getDocs(q)
...
export const deleteServidorFromFirebase = async (servidorId) => {
  ...
  try {
    const servidorRef = doc(db, SERVIDORES_COLLECTION, `${USER_ID}_${servidorId}`)
...
export const setupRealtimeSync = (onSyncStatusChange) => {
  ...
  const qClientes = query(collection(db, CLIENTES_COLLECTION), where('userId', '==', USER_ID))
  const unsubscribeClientes = onSnapshot(qClientes, async (snapshot) => {
  ...
  const qServidores = query(collection(db, SERVIDORES_COLLECTION), where('userId', '==', USER_ID))
  const unsubscribeServidores = onSnapshot(qServidores, async (snapshot) => {
    if (isSyncing) return // Evitar loop
